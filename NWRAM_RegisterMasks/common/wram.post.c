...
void WRAMSelfTest(SWRAMPOSTRESULTS *post) 
{
  ...
  WRAMSelfTest_Write(post) ;
    ...
    WRAMSelfTest_RegisterMasks(post) ;
  #endif
}
...
void WRAMSelfTest_RegisterMasks(SWRAMPOSTRESULTS *post) 
{
  ...
  // we do this in two parts: One bit at a time and all at once
  // this way we might find limited fields, which can be set to a certain range
  // but would not set all bits on the highest possible value
  //
  // 1. Setup everything for the lowest possible impact
  ...
  // 2. Check all the bank registers
  for (int bank = 0;bank < 20;bank++)
  {
    uint8_t maskSingleSet = 0 ;
    uint8_t maskSingleClear = 0 ;
    for (int bit=0;bit<8;bit++)
    {
      ((volatile uint8_t *)0x04004040)[bank] = (1 << bit) ;
      if (((volatile uint8_t *)0x04004040)[bank] & (1 << bit))
      {
        maskSingleSet |= (1 << bit) ;
        ((volatile uint8_t *)0x04004040)[bank] &= ~(1 << bit) ;
        if (!(((volatile uint8_t *)0x04004040)[bank] & (1 << bit)))
        {
          maskSingleClear |= (1 << bit) ;
        }
      }
    }
    post->wramBankSingleSetMask[bank] = maskSingleSet ;
    post->wramBankSingleClearMask[bank] = maskSingleClear ;    
    // as a whole:
